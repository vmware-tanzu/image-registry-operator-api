...
func SpokeHubSpoke(g gomega.Gomega, input FuzzTestFuncInput) {
	...
	for i := 0; i < 10000; i++ {
		...
		g.ExpectWithOffset(1, spokeBefore.ConvertTo(hubCopy)).To(gomega.Succeed())
		...
		g.ExpectWithOffset(1, spokeAfter.ConvertFrom(hubCopy)).To(gomega.Succeed())
		...
		g.ExpectWithOffset(1, apiequality.Semantic.DeepEqual(spokeBefore, spokeAfter)).To(gomega.BeTrue(), cmp.Diff(spokeBefore, spokeAfter))
	}
}

func HubSpokeHub(g gomega.Gomega, input FuzzTestFuncInput) {
	fuzzer := GetFuzzer(input.Scheme, input.FuzzerFuncs...)
	...
	for i := 0; i < 10000; i++ {
		...
		g.ExpectWithOffset(1, dstCopy.ConvertFrom(hubBefore)).To(gomega.Succeed())

		// Convert spoke back to hub and check if the resulting hub is equal to the hub before the round trip
		hubAfter := input.Hub.DeepCopyObject().(ctrlconversion.Hub)
		g.ExpectWithOffset(1, dstCopy.ConvertTo(hubAfter)).To(gomega.Succeed())

		if input.HubAfterMutation != nil {
			input.HubAfterMutation(hubAfter)
		}

		g.ExpectWithOffset(1, apiequality.Semantic.DeepEqual(hubBefore, hubAfter)).To(gomega.BeTrue(), cmp.Diff(hubBefore, hubAfter))
	}
}
